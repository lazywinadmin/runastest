...
.EXAMPLE
    Get-RunAs2 -User Ted -Password Password1 -Domain MYDOMAIN -Command C:\Temp\Runme.exe                   

.EXAMPLE
    Get-RunAs2 -User Ted -Password Password1 -Domain MYDOMAIN -Command C:\Windows\system32\WindowsPowershell\v1.0\powershell.exe -Args " -exec bypass -e Tjsksdsadsa"    
...
"@  
    ...
    if (($env:username -eq "$($env:computername)$")) {
        ...
        # Inspired from: https://github.com/FuzzySecurity/PowerShell-Suite/blob/master/Get-RunAs2.ps1
    	# StartupInfo Struct
	    $StartupInfo = New-Object STARTUPINFO
	    $StartupInfo.dwFlags = 0x00000001
	    $StartupInfo.wShowWindow = 0x0001
